...
function renderProducts(categorySlug = 'all', searchQuery = '') {
    ...
    displayProducts.forEach(p => {
        ...
        if (window.productViewMode === 'list') {
            ...
            const iconEmoji = `<img src="${productImg}" alt="${p.name}" style="width: 100%; height: 100%; object-fit: contain; display: block; padding: 10px; background: #fafafa;">`;
...
function setupHeaderActions() {
    ...
    const isHomePage = window.location.pathname === '/' || window.location.pathname.endsWith('/') || window.location.pathname === '';
    ...
    if (searchInput) {
        const triggerSearchRedirect = () => {
            ...
            if (query.length > 0) {
                window.location.href = `/hasil_pencarian?query=${encodeURIComponent(query)}`;
            }
        };
